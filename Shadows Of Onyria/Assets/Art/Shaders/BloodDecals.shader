...
Shader "BloodDecals"
{
	Properties
	{
		...
		_CurrentValue("CurrentValue", Range( 0 , 1)) = 0
		_PositionX("PositionX", Float) = 0
		_PositionY("PositionY", Float) = 0
		[ASEEnd]_PositionZ("PositionZ", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 uv_Decal = packedInput.ase_texcoord5.xy * _Decal_ST.xy + _Decal_ST.zw;
				float4 tex2DNode1 = tex2D( _Decal, uv_Decal );
				
				float3 ase_worldPos = GetAbsolutePositionWS( positionRWS );
				float3 surf_pos107_g1 = ase_worldPos;
				float3 surf_norm107_g1 = normalWS;
				float height107_g1 = tex2DNode1.r;
				...
				float3 appendResult41 = (float3(_PositionX , _PositionY , _PositionZ));
				float lerpResult28 = lerp( tex2DNode1.r , 0.0 , saturate( pow( abs( ( distance( appendResult41 , ase_worldPos ) / ( ( _Radius * _ObjectScale ) * _CurrentValue ) ) ) , _FallOff ) ));
				
				surfaceDescription.Albedo = ( _Color * tex2DNode1 ).rgb;
				surfaceDescription.Normal = worldToTangentDir42_g1;
				...
				surfaceDescription.Alpha = lerpResult28;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_Decal = packedInput.ase_texcoord.xy * _Decal_ST.xy + _Decal_ST.zw;
				float4 tex2DNode1 = tex2D( _Decal, uv_Decal );
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_Decal = packedInput.ase_texcoord1.xy * _Decal_ST.xy + _Decal_ST.zw;
				float4 tex2DNode1 = tex2D( _Decal, uv_Decal );
				float3 appendResult41 = (float3(_PositionX , _PositionY , _PositionZ));
				float3 ase_worldPos = GetAbsolutePositionWS( positionRWS );
				float lerpResult28 = lerp( tex2DNode1.r , 0.0 , saturate( pow( abs( ( distance( appendResult41 , ase_worldPos ) / ( ( _Radius * _ObjectScale ) * _CurrentValue ) ) ) , _FallOff ) ));
		...
		Pass
		{
					...
					)
			{
				...
				float3 surf_pos107_g1 = ase_worldPos;
				float3 surf_norm107_g1 = normalWS;
				float2 uv_Decal = packedInput.ase_texcoord3.xy * _Decal_ST.xy + _Decal_ST.zw;
				float4 tex2DNode1 = tex2D( _Decal, uv_Decal );
				float height107_g1 = tex2DNode1.r;
				float scale107_g1 = _NormalStrength;
				float3 localPerturbNormal107_g1 = PerturbNormal107_g1( surf_pos107_g1 , surf_norm107_g1 , height107_g1 , scale107_g1 );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_Decal = packedInput.ase_texcoord7.xy * _Decal_ST.xy + _Decal_ST.zw;
				float4 tex2DNode1 = tex2D( _Decal, uv_Decal );
...
WireConnection;5;110;7;0
WireConnection;6;0;2;0
WireConnection;6;1;1;0
WireConnection;8;0;6;0
WireConnection;8;1;5;40
WireConnection;8;7;19;0
WireConnection;8;9;28;0
ASEEND*/
//CHKSM=2505BE313A1498986F00DA33ED5C4007BBC4492E